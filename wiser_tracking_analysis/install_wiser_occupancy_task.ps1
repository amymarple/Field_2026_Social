...
param(
    ...
    [string]$DbPath    = 'D:\Wiser\data\1stcohort_mice_2026.sqlite',
    ...
    [int]$AtMinute     = 5,
    [string]$Identities = '',   # per-cohort rat_identities CSV (tag ids are reused across cohorts)
    [switch]$RunNow
)
...
if ($Identities) {
    if (-not (Test-Path -LiteralPath $Identities)) { throw "Identities CSV not found: $Identities" }
    $argList += @('--identities', ('"{0}"' -f $Identities))
}
$argList += '--backfill'
$arg = $argList -join ' '
