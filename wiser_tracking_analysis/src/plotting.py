...
# Process-wide override, set once by a driver script (see set_identity_path).
# Needed because WISER tag ids are REUSED across cohorts: the same physical tags
# (305a/305c/3062/306b/3077/3079) moved from the 2026 rats to the mice on
# 2026-07-15, so shortid alone does not identify an animal — the caller must say
# which cohort's mapping applies.
_IDENTITY_PATH_OVERRIDE: Path | None = None


def set_identity_path(path: Path | str | None) -> None:
    """Point every later load_rat_identities() call at *path* (None = default)."""
    global _IDENTITY_PATH_OVERRIDE
    _IDENTITY_PATH_OVERRIDE = Path(path) if path else None


def load_rat_identities(path: Path | None = None) -> dict:
    """shortid (str) -> identity dict. Empty if the mapping file is absent."""
    return _load_identities_cached(
        str(path or _IDENTITY_PATH_OVERRIDE or _DEFAULT_IDENTITY_CSV))
...
def plot_hourly_scatter(df: pd.DataFrame,
    ...
    if time_im is not None:
        ...
        # "window" not "hour": this plot is also used for arbitrary --from/--to ranges.
        cbar.set_label("time within window (early → late)", fontsize=8)
