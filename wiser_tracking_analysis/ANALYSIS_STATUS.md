...
| Sleep-site WISER↔CV cross-val (Direction 3) | `scripts/analyze_sleep_site_cv_crossval.py` | ⚠️ | Shelter-occupancy agreement vs CV (CH05/CH06). **Binning bug fixed 2026-07-06** (pandas `[ms]` collapsed a day to 1 bin); 07-02 rerun now yields 960 bins / 42 episodes (19 hc) | Only the 2 shelters; **CV↔WISER alignment UNVERIFIED (joint κ=0.20, lag 0 s)** → recall/precision are diagnostics, not behavior; CH05 recall gap sits on *clear* glass ⇒ wall-edge lower bound, not proven optical failure; older 6/29–6/30 κ (0.66 / 0.68–0.82) predate the fix (`[ns]` pandas) → re-confirm; interpret only after alignment resolved | [plan](../implementation_plan/2026-07-02-sleep-site-cv-crossval.md) · [log](../change_log/2026-07-02-sleep-site-cv-crossval.md) · [binning fix](../change_log/2026-07-06-wiser-binning-resolution-fix.md) |
